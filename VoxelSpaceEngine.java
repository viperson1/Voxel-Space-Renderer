...
public class VoxelSpaceEngine implements KeyListener {
	...
	VoxelSpaceEngine() throws IOException {
		...
		inputHeightMap = new File("Maps/D13.png");
		imageHeightMap = ImageIO.read(inputHeightMap);
		inputColorMap = new File("Maps/C13.png");
	...
	public static void main(String[] args) throws IOException, AWTException {
		...
		while(running) { //game loop
			//set camera position;
			engine.cameraPosZ = engine.posZ; //- (((engine.horizon / engine.renderedScreenHeight) - .5) * (engine.heightScale / 48));
			engine.cameraPosX = engine.posX; //+ Math.sin(Math.toRadians(engine.direction)) * engine.cameraDistance;
			engine.cameraPosY = engine.posY; //+ Math.cos(Math.toRadians(engine.direction)) * engine.cameraDistance;
	...
	int[] renderFrame() {
		...
		for(int column = 0; column < renderedScreenWidth; column++) {
			...
			while(inBounds && renderDist < drawDist) {
				for(int i = 0; i < steps/*(1 << (jumpDist - 1))*/; i++) {
				...
				//if(quickCheck || renderDist > 75) {
					if (side == 0) {
						renderDist = ((mapSquare[0] - cameraPosX) + ((1 - stepX) * 0.5)) / rayDir[0];
					}
					else {
						renderDist = ((mapSquare[1] - cameraPosY) + ((1 - stepY) * 0.5)) / rayDir[1];
					}
				/*}
				else {
					double distX = testSquareX - cameraPosX;
					double distY = testSquareY - cameraPosY;
				...
				if(testSquareX >= 0 && testSquareX < mapWidth && testSquareY >= 0 && testSquareY < mapWidth) {
					if(!quickCheck || heightMap[testSquareX][testSquareY] > heightBuffer) {
						int heightOnScreen = renderedScreenHeight - (int)(((cameraPosZ + cameraHeight) - 
								(heightMap[testSquareX][testSquareY])
								) / renderDist * heightScale + horizon);
